...
window.wp.ajax.post("csf-reset",{unique:c.data("unique"),nonce:c.data("nonce")}).done(function(e){window.location.reload(!0)}).fail(function(e){alert('重置失败，可能是您的服务器开启了防火墙，请求被拦截，若未开启防火墙，可前往插件页面将BearHoneyCore插件卸载然后重新启用后重试')})});d.on("click",function(g){g.preventDefault();confirm("\u786e\u5b9a\u5417")&&window.wp.ajax.post("csf-import",{unique:d.data("unique"),nonce:d.data("nonce"),data:a.find(".csf-import-data").val()}).done(function(e){window.location.reload(!0)}).fail(function(e){alert('导入数据失败，可能是您的服务器开启了防火墙，请求被拦截，若未开启防火墙，可前往插件页面将BearHoneyCore插件卸载然后重新启用后重试')})})})};b.fn.csf_field_background=function(){return this.each(function(){b(this).find(".csf--background-image").csf_reload_script()})};
